...
const NODE_CLASS = "MiniMaxH3Easy_SatoDive";
const CONTEXT_SEGMENTS_CLASS = "MiniMaxH3EasyContextSegments_SatoDive";
const SELECTED_VIDEO_CONTEXT_CLASS = "MiniMaxH3EasySelectedVideoContext_SatoDive";
const LOADER_CLASS = "MiniMaxH3EasyLoader_SatoDive";
const ADAPTER_CLASS = "MiniMaxH3EasyModelAdapter_SatoDive";
const MEDIA_LOADER_CLASS = "MiniMaxH3EasyMediaLoader_SatoDive";
const MEDIA_BRIDGE_CLASS = "MiniMaxH3EasyMediaBridge_SatoDive";
const MEDIA_SPLITTER_CLASS = "MiniMaxH3EasyMediaSplitter_SatoDive";
const SEGMENT_RENDER_CLASS = "MiniMaxH3EasySegmentRender_SatoDive";
const SEGMENT_REFINE_CLASS = "MiniMaxH3EasySegmentRefine_SatoDive";
const SEGMENT_SAMPLE_SETUP_CLASS = "MiniMaxH3EasySegmentSampleSetup_SatoDive";
const SEGMENT_STEP_CLASS = "MiniMaxH3EasySegmentStep_SatoDive";
const SEGMENT_COLLECT_CLASS = "MiniMaxH3EasySegmentCollect_SatoDive";
const SEGMENT_DECODE_CLASS = "MiniMaxH3EasySegmentDecode_SatoDive";
const OUTPUT_CLASS = "MiniMaxH3EasyOutput_SatoDive";
...
const CONTINUITY_NATIVE = "native_guide";
...
const TEXT = {
    ...
    mainTitle: "MiniMax H3 Easy - SatoDive",
    selectedVideoContextTitle: ZH_BROWSER ? "MiniMax H3 Easy 已选视频上下文 - SatoDive" : "MiniMax H3 Easy Selected Video Context - SatoDive",
    loaderTitle: ZH_BROWSER ? "MiniMax H3 Easy \u52a0\u8f7d\u5668 - SatoDive" : "MiniMax H3 Easy Loader - SatoDive",
    adapterTitle: ZH_BROWSER ? "MiniMax H3 Easy \u6a21\u578b\u4e2d\u8f6c - SatoDive" : "MiniMax H3 Easy Model Bridge - SatoDive",
    mediaBridgeTitle: ZH_BROWSER ? "MiniMax H3 Easy \u5a92\u4f53\u4e2d\u8f6c - SatoDive" : "MiniMax H3 Easy Media Bridge - SatoDive",
    mediaLoaderTitle: ZH_BROWSER ? "MiniMax H3 Easy \u5a92\u4f53\u52a0\u8f7d\u5668 - SatoDive" : "MiniMax H3 Easy Media Loader - SatoDive",
    mediaSplitterTitle: ZH_BROWSER ? "MiniMax H3 Easy \u5a92\u4f53\u62c6\u5206 - SatoDive" : "MiniMax H3 Easy Media Splitter - SatoDive",
    ...
    outputTitle: ZH_BROWSER ? "MiniMax H3 Easy \u8f93\u51fa - SatoDive" : "MiniMax H3 Easy Output - SatoDive",
    ...
    refImageStrength: ZH_BROWSER ? "参考图强度" : "Reference image strength",
    refVideoStrength: ZH_BROWSER ? "参考视频强度" : "Reference video strength",
    refStrengths: ZH_BROWSER ? "参考逐项强度（按序号）" : "Reference strengths (by order)",
    enableDetailDaemon: ZH_BROWSER ? "细节守护 (Detail Daemon)" : "Detail Daemon",
    detailStrength: ZH_BROWSER ? "细节强度" : "Detail strength",
    ...
    segmentRenderTitle: ZH_BROWSER ? "MiniMax H3 Easy \u5206\u6bb5\u91c7\u6837 - SatoDive" : "MiniMax H3 Easy Segment Sample - SatoDive",
    segmentRefineTitle: ZH_BROWSER ? "MiniMax H3 Easy \u5206\u6bb5\u4e8c\u91c7 - SatoDive" : "MiniMax H3 Easy Segment Refine - SatoDive",
    segmentSampleSetupTitle: ZH_BROWSER ? "MiniMax H3 Easy \u91c7\u6837\u8bbe\u7f6e - SatoDive" : "MiniMax H3 Easy Sample Setup - SatoDive",
    segmentSampleSetupPort: ZH_BROWSER ? "\u91c7\u6837\u8bbe\u7f6e" : "Sample Setup",
    segmentStepTitle: ZH_BROWSER ? "MiniMax H3 Easy \u5206\u6bb5\u6b65\u9aa4 - SatoDive" : "MiniMax H3 Easy Segment Step - SatoDive",
    segmentCollectTitle: ZH_BROWSER ? "MiniMax H3 Easy \u5206\u6bb5\u6c47\u603b - SatoDive" : "MiniMax H3 Easy Segment Collect - SatoDive",
    segmentDecodeTitle: ZH_BROWSER ? "MiniMax H3 Easy \u5206\u6bb5\u89e3\u7801 - SatoDive" : "MiniMax H3 Easy Segment Decode - SatoDive",
    contextSegmentsTitle: ZH_BROWSER ? "MiniMax H3 Easy \u4e0a\u4e0b\u6587\u5206\u6bb5 - SatoDive" : "MiniMax H3 Easy Context Segments - SatoDive",
    ...
    seedVideoStrength: ZH_BROWSER ? "\u79cd\u5b50\u89c6\u9891\u5f3a\u5ea6" : "Seed Video Strength",
    seedVideoInput: ZH_BROWSER ? "\u8fde\u7eed\u89c6\u9891" : "seed_video (continuation)",
    seedRefVideoInput: ZH_BROWSER ? "\u53c2\u8003\u89c6\u9891" : "seed_ref_video (reference)",
    seedVideoReservedSeconds: ZH_BROWSER ? "\u79cd\u5b50\u4e0a\u4e0b\u6587\u79d2\u6570" : "Seed Context Seconds (0 = use Context frames)",
...
const OPTION_DEFS = {
    ...
    continuity_mode: {
        [CONTINUITY_NATIVE]: "Native Guide",
...
function isCustomAspectRatio(node) {
    return String(getWidgetValue(node, "aspect_ratio", "16:9")) === "Custom";
}
        ...
        for (const node of app.graph?._nodes || []) {
            ...
            if (isContextSegmentsNode(node)) {
                ...
                promptNode.inputs.continuity_mode = canonicalOption(
                    "continuity_mode",
                    getWidgetValue(node, "continuity_mode", CONTINUITY_NATIVE),
                );
            ...
            if (!isSelectedVideoContextNode(node)) {
                preserveLinkedPromptInput(promptNode, node, "enable_detail_daemon", asBoolean(getWidgetValue(node, "enable_detail_daemon", false)));
                preserveLinkedPromptInput(promptNode, node, "detail_strength", Number(getWidgetValue(node, "detail_strength", 0.0)));
                for (let i = 1; i <= 9; i += 1) {
                    const name = `ref_image${i}_strength`;
                    const widget = getWidget(node, name);
                    if (widget && widget.value !== undefined) {
                        promptNode.inputs[name] = Number(widget.value);
                    }
                }
                for (let v = 1; v <= 3; v += 1) {
                    const name = `ref_video${v}_strength`;
                    const widget = getWidget(node, name);
                    if (widget && widget.value !== undefined) {
                        promptNode.inputs[name] = Number(widget.value);
                    }
                ...
                for (const opt of options) {
                    if (opt.type === "image") {
                        const val = Number(getWidgetValue(node, `ref_image${opt.ordinal}_strength`, 1.0));
                        strengthsList.push(Number.isFinite(val) ? val : 1.0);
                    } else if (opt.type === "video") {
                        const val = Number(getWidgetValue(node, `ref_video${opt.ordinal}_strength`, 1.0));
                        strengthsList.push(Number.isFinite(val) ? val : 1.0);
                    }
                }
                if (strengthsList.length) {
                    promptNode.inputs.ref_strengths = strengthsList.join(", ");
                }
            }
        }
        return promptData;
...
// seed_ref_video is Context Segments' reference input. It is a plain native
// input rather than a "media" virtual link, so it would otherwise never appear
// in the @-mention pool and @Video1 would resolve to nothing even though the
// backend does place it at <Video 1> (see nodes.py's Context Segments
// `generate`, which prepends it to the media items).
//
// This deliberately reads seed_ref_video, NOT seed_video. Those two inputs are
// a hard split: seed_video is the continuation anchor only and is never added
// to the reference library, so it can never be <Video 1>. Offering it here
// would show a resolved @Video1 chip for a reference the backend does not have.
function seedVideoMentionOption(node) {
    if (!isContextSegmentsNode(node)) return null;
    const input = findInputByName(node, "seed_ref_video");
    if (input?.link == null) return null;
    const graph = node.graph || app.graph;
    const link = getNativeGraphLink(graph, input.link);
    if (!link) return null;
    const rawSourceId = link.origin_id ?? link.originId ?? link.from_id ?? link.fromId;
    const sourceNode = link.origin_node || link.originNode || link.fromNode || link.sourceNode
        || graph?.getNodeById?.(Number(rawSourceId));
    if (!sourceNode) return null;
    const sourceId = rawSourceId ?? sourceNode.id;
    if (sourceId == null) return null;
    const rawSourceSlot = link.origin_slot ?? link.originSlot ?? link.from_slot ?? link.fromSlot ?? 0;
    const sourceSlot = Number.isFinite(Number(rawSourceSlot)) ? Number(rawSourceSlot) : 0;
    watchMediaSourceNode(sourceNode);
    const filename = sourceFilename(sourceNode, "video");
    const fullLabel = filename || sourceLabel(sourceNode);
    const mode = referenceMentionMode(node);
    const label = mode === "index" ? `${LABELS.video || "video"}1` : truncateMentionLabel(fullLabel);
    return {
        type: "video",
        tag: "<Video 1>",
        ...
        ordinal: 1,
        referenceMode: mode,
        source: sourceLabel(sourceNode),
        sourceId: Number(sourceId),
        sourceSlot,
        mediaIndex: 0,
        previewUrl: sourcePreviewUrl(sourceNode, "video"),
        isSeedVideo: true,
    };
}
...
function mentionOptions(node) {
    ...
    const seedVideo = seedVideoMentionOption(node);
    ...
    if (loader && isMediaLoader(loader)) {
        const loaderOptions = mediaLoaderMentionOptions(loader, node);
        return seedVideo ? [seedVideo, ...bumpVideoOrdinals(loaderOptions)] : loaderOptions;
    }
    ...
    const counts = { image: 0, video: 0, audio: 0 };
    const mode = referenceMentionMode(node);
    const mapped = orderedLinks.map(({ link, mediaIndex }) => {
        const type = String(link.media_type || "image");
    ...
    return seedVideo ? [seedVideo, ...bumpVideoOrdinals(mapped)] : mapped;
}

// seed_video always claims <Video 1> (matching the backend, which prepends
// it to the media items before assigning ordinals) -- so every other video
// mention shifts down by one to stay in sync with what the backend will
// actually resolve @VideoN to.
function bumpVideoOrdinals(options) {
    return options.map((option) => {
        if (option.type !== "video") return option;
        const ordinal = Number(option.ordinal || 0) + 1;
        const tag = `<Video ${ordinal}>`;
        const isIndexMode = option.referenceMode === "index";
        const label = isIndexMode ? `${LABELS.video || "video"}${ordinal}` : option.label;
        const token = isIndexMode ? `@${label}` : option.token;
        return { ...option, ordinal, tag, label, token };
    });
}
...
function contextFrameGrid(node) {
    const mode = canonicalOption("continuity_mode", getWidgetValue(node, "continuity_mode", CONTINUITY_NATIVE));
...
function positionReferenceStrengthWidgets(node) {
    if (!Array.isArray(node?.widgets)) return;
    const refMentionWidget = getWidget(node, "reference_mention_mode");
    if (!refMentionWidget) return;
    const startIndex = node.widgets.indexOf(refMentionWidget);
    if (startIndex < 0) return;
    let targetIndex = startIndex + 1;
    for (let i = 1; i <= 9; i += 1) {
        const w = getWidget(node, `ref_image${i}_strength`);
        if (w) {
            const cur = node.widgets.indexOf(w);
            if (cur >= 0 && cur !== targetIndex) {
                node.widgets.splice(cur, 1);
                node.widgets.splice(targetIndex, 0, w);
            }
            targetIndex += 1;
        }
    }
    for (let v = 1; v <= 3; v += 1) {
        const w = getWidget(node, `ref_video${v}_strength`);
        if (w) {
            const cur = node.widgets.indexOf(w);
            if (cur >= 0 && cur !== targetIndex) {
                node.widgets.splice(cur, 1);
...
function syncReferenceStrengthWidgets(node, { adjustHeight = true } = {}) {
    if (!node || !isTarget(node)) return false;
    // isReferenceMode() only returns true when the node's own `mode` widget
    // equals MODE_REFERENCE/MODE_DIGITAL_HUMAN. Context Segments (and
    // segmented Selected Video Context) fix `mode` to MODE_SEGMENTS, so that
    // check was always false there and this function hid every ref_image/
    // ref_video strength widget unconditionally, no matter what was linked.
    // mediaReferenceMode() already has the Context-Segments-aware branch
    // mentionOptions() itself relies on -- use the same gate here so the
    // widgets actually appear (and hide) in step with the @-mentions.
    const isRef = mediaReferenceMode(node);
    const options = isRef ? mentionOptions(node) : [];
    const imageOptions = options.filter((opt) => opt.type === "image");
    const videoOptions = options.filter((opt) => opt.type === "video");

    positionReferenceStrengthWidgets(node);
    ...
    for (let i = 1; i <= 9; i += 1) {
        ...
        const attached = imageOptions.find((opt) => opt.ordinal === i);
        if (attached && isRef) {
            if (!widget && typeof node.addWidget === "function") {
                widget = node.addWidget("number", name, 1.0, (val) => {
                    if (widget._state) widget._state.value = val;
                }, { min: 0.0, max: 5.0, step: 0.05, precision: 2 });
            }
            if (widget) {
                const cleanName = truncateMentionLabel(attached.fullLabel || attached.label || `Image ${i}`, 18);
                widget.label = ZH_BROWSER
                    ? `参考图 ${i} 强度 (${cleanName})`
                    : `Ref Image ${i} Strength (${cleanName})`;
                changed.push(setConditionalWidgetVisible(node, widget, true, { adjustHeight }));
            }
        } else if (widget) {
            changed.push(setConditionalWidgetVisible(node, widget, false, { adjustHeight }));
        }
    }
    ...
    for (let v = 1; v <= 3; v += 1) {
        const name = `ref_video${v}_strength`;
        let widget = getWidget(node, name);
        const attached = videoOptions.find((opt) => opt.ordinal === v);
        if (attached && isRef) {
            if (!widget && typeof node.addWidget === "function") {
                widget = node.addWidget("number", name, 1.0, (val) => {
                    if (widget._state) widget._state.value = val;
            ...
            if (widget) {
                const cleanName = truncateMentionLabel(attached.fullLabel || attached.label || `Video ${v}`, 18);
                widget.label = ZH_BROWSER
                    ? `参考视频 ${v} 强度 (${cleanName})`
                    : `Ref Video ${v} Strength (${cleanName})`;
                changed.push(setConditionalWidgetVisible(node, widget, true, { adjustHeight }));
            }
        } else if (widget) {
            changed.push(setConditionalWidgetVisible(node, widget, false, { adjustHeight }));
...
function syncModeWidgets(node, { adjustHeight = true } = {}) {
    ...
    // Inline context segments on the main MiniMaxH3Easy node (not the standalone ContextSegments node)
    const inlineSegmentsEnabled = !contextNode && !selectedVideoContext && asBoolean(getWidgetValue(node, "enable_context_segments", false));
    const changed = [
        syncContextLengthWidget(node),
        setConditionalWidgetVisible(node, getWidget(node, "mode"), !contextNode, { adjustHeight }),
        setConditionalWidgetVisible(node, getWidget(node, "audio_mode"), contextNode || inlineSegmentsEnabled, { adjustHeight }),
        ...
        setConditionalWidgetVisible(node, getWidget(node, "ref_image_strength"), false, { adjustHeight }),
        setConditionalWidgetVisible(node, getWidget(node, "ref_video_strength"), false, { adjustHeight }),
        setConditionalWidgetVisible(node, getWidget(node, "ref_strengths"), false, { adjustHeight }),
        // Removed feature (per user request): these three still exist in the
        // Python schema, inert, purely so already-saved workflows don't
        // have their later widget values (seed_video_strength) shift into
        // the wrong slot on load. Force-hidden here the same way the
        // superseded ref_image_strength/ref_video_strength/ref_strengths
        // widgets above are.
        setConditionalWidgetVisible(node, getWidget(node, "ref_video1_range"), false, { adjustHeight }),
        setConditionalWidgetVisible(node, getWidget(node, "ref_video2_range"), false, { adjustHeight }),
        setConditionalWidgetVisible(node, getWidget(node, "ref_video3_range"), false, { adjustHeight }),
        syncReferenceStrengthWidgets(node, { adjustHeight }),
        setConditionalWidgetVisible(node, getWidget(node, "aspect_ratio"), !isCustomResolution(node), { adjustHeight }),
        setConditionalWidgetVisible(node, getWidget(node, "width"), isCustomResolution(node) || isCustomAspectRatio(node), { adjustHeight }),
        setConditionalWidgetVisible(node, getWidget(node, "height"), isCustomResolution(node) || isCustomAspectRatio(node), { adjustHeight }),
        ...
        // Retired per user request: "Segment seconds" is no longer a UI control.
        // "seconds" above it is now the single, always-editable duration control.
        setConditionalWidgetVisible(node, getWidget(node, "segment_seconds"), false, { adjustHeight }),
        setConditionalWidgetVisible(node, getWidget(node, "context_length"), segmentContextNode || inlineSegmentsEnabled, { adjustHeight }),
        setConditionalWidgetVisible(node, getWidget(node, "continuity_mode"), segmentContextNode || inlineSegmentsEnabled, { adjustHeight }),
        setConditionalWidgetVisible(
            node,
            getWidget(node, "seed_video_strength"),
            segmentContextNode && node.inputs?.find((inp) => inp.name === "seed_video")?.link != null,
            { adjustHeight },
        ),
        ...
        setConditionalWidgetVisible(node, getWidget(node, "detail_strength"), asBoolean(getWidgetValue(node, "enable_detail_daemon", false)), { adjustHeight }),
    ].some(Boolean);
    // "Segment seconds" no longer drives "seconds" (that sync used to
    // overwrite whatever the user typed in "seconds" with a sum pulled from
    // the now-hidden segment_seconds widget) -- "seconds" is the direct,
    // user-editable duration control everywhere now, so nothing needs to
    // sync into it here any more.
...
function ensurePromptEditor(node) {
    ...
    editor.addEventListener("paste", (event) => {
        ...
        // Reference tokens resolve against mentionOptions(node), which is
        // computed live from the node's media links. Right after a paste those
        // links may not be normalized yet, so there are no candidates to match:
        // the text lands plain, no chips appear, and the stored prompt doc has
        // no mention parts. Pasting a second time works only because the links
        // have settled by then. Detect that miss and re-parse once on the next
        // tick instead of making the user do it by hand.
        const chipsBefore = editor.querySelectorAll(".h3-mention-chip").length;
        const hasMentionTokens = /@(?:Image|Picture|Video|Audio)\s?\d+|<\s*(?:picture|video|audio)\s*\d+\s*>/i.test(text);
        insertTextWithMentionChips(node, editor, text);
        syncPromptFromEditor(node);
        pushPromptHistory(node);
        syncMentionMenuToCaret(node, editor);
        if (hasMentionTokens && editor.querySelectorAll(".h3-mention-chip").length === chipsBefore) {
            setTimeout(() => {
                try {
                    if (!node.__h3Editor || isRawPromptMode(node)) return;
                    if (editor.querySelectorAll(".h3-mention-chip").length !== chipsBefore) return;
                    if (!mentionOptions(node).length) return;
                    const widget = getWidget(node, "prompt");
                    const full = String(widget?.value ?? editor.textContent ?? "");
                    if (!full) return;
                    editor.textContent = "";
                    const selection = window.getSelection?.();
                    const range = document.createRange();
                    ...
                    selection?.removeAllRanges();
                    selection?.addRange(range);
                    insertTextWithMentionChips(node, editor, full);
                    syncPromptFromEditor(node);
                } catch (err) {
                    console.warn("[MiniMax H3 Easy] mention re-parse after paste failed", err);
                }
            }, 0);
        }
    });
...
function repairConfiguredWidgetValues(node, info) {
    ...
    const defaults = {
        ...
        enable_detail_daemon: false,
        detail_strength: 0.0,
        ref_image_strength: 1.0,
        ref_video_strength: 1.0,
        ref_strengths: "",
    };
    for (let i = 1; i <= 9; i += 1) defaults[`ref_image${i}_strength`] = 1.0;
    for (let v = 1; v <= 3; v += 1) defaults[`ref_video${v}_strength`] = 1.0;
    ...
    if (isSelectedVideoContextNode(node)) {
        const selectedDefaults = {
            ...
            continuity_mode: CONTINUITY_NATIVE,
        ...
        for (let i = 1; i <= 9; i += 1) selectedDefaults[`ref_image${i}_strength`] = 1.0;
        for (let v = 1; v <= 3; v += 1) selectedDefaults[`ref_video${v}_strength`] = 1.0;

        // Current selected-video schema is:
        // mode, prompt, segment_mode, segment_cuts, context_length,
        // continuity_mode, advanced, then the advanced controls, then
        // enable_detail_daemon, detail_strength (appended last so older
        // saved workflows without the Detail Daemon controls still restore
        // every earlier value at the same positional index).
        //
        ...
        const selectedValues = {
            ...
            enable_detail_daemon: booleanValue("enable_detail_daemon", values[14], selectedDefaults.enable_detail_daemon),
            detail_strength: numberValue("detail_strength", values[15], selectedDefaults.detail_strength),
            ref_image_strength: numberValue("ref_image_strength", undefined, selectedDefaults.ref_image_strength),
            ref_video_strength: numberValue("ref_video_strength", undefined, selectedDefaults.ref_video_strength),
            ref_strengths: stringValue("ref_strengths", undefined, selectedDefaults.ref_strengths),
        };
        for (let i = 1; i <= 9; i += 1) {
            const n = `ref_image${i}_strength`;
            const val = named[n] !== undefined ? Number(named[n]) : (Number(getWidget(node, n)?.value) || 1.0);
            selectedValues[n] = Number.isFinite(val) ? val : 1.0;
        }
        for (let v = 1; v <= 3; v += 1) {
            const n = `ref_video${v}_strength`;
            const val = named[n] !== undefined ? Number(named[n]) : (Number(getWidget(node, n)?.value) || 1.0);
            selectedValues[n] = Number.isFinite(val) ? val : 1.0;
        }
        ...
        info.widgets_values = [
            ...
            selectedValues.enable_detail_daemon,
            selectedValues.detail_strength,
        ];
        info.widgets_values_named = {
    ...
    // NOTE: there is a "custom_ratio" (STRING) widget positioned right after
    // aspect_ratio in the Python INPUT_TYPES for both this node and the
    // Context Segments node. It occupies its own slot in widgets_values, so
    // every field from "width" onward is shifted one position later than it
    // would be without it. Forgetting to count it here previously caused
    // every widget after aspect_ratio to be read from the wrong slot on
    // every onConfigure (e.g. every workflow-tab switch) -- most visibly,
    // "advanced" ended up reading the raw "seconds" slot, and any nonzero
    // seconds value made asBoolean() report the checkbox as enabled.
    const widthIndex = contextSegments ? 6 : 5;
    const heightIndex = contextSegments ? 7 : 6;
    const secondsIndex = contextSegments ? 8 : 7;
    const segmentSecondsIndex = contextSegments ? 9 : -1;
    const contextLengthIndex = contextSegments ? 10 : -1;
    const continuityModeIndex = contextSegments ? 11 : -1;
    // @ag Detect the new positional layout from the shifted advanced boolean; named metadata may accompany legacy arrays.
    const contextTransitionOffset = contextSegments && typeof values[13] === "boolean" ? 1 : 0;
    const transitionSecondsIndex = contextSegments && contextTransitionOffset ? 12 : -1;
    const advancedIndex = contextSegments ? 12 + contextTransitionOffset : 8;
    const fpsIndex = contextSegments ? 13 + contextTransitionOffset : 9;
    const keyframeRoleIndex = contextSegments ? 14 + contextTransitionOffset : 10;
    const refImageSizeIndex = contextSegments ? 15 + contextTransitionOffset : 11;
    const referenceMentionModeIndex = contextSegments ? 16 + contextTransitionOffset : 12;
    const promptOptimizerSceneGuideIndex = contextSegments ? 18 + contextTransitionOffset : 14;
    const contextOptimizerModeIndex = contextSegments ? 19 + contextTransitionOffset : -1;
    const contextOptimizerConcurrencyIndex = contextSegments ? 20 + contextTransitionOffset : -1;

    const enableDetailDaemonIndex = contextSegments ? 21 + contextTransitionOffset : 15;
    const detailStrengthIndex = contextSegments ? 22 + contextTransitionOffset : 16;
    const refImageStrengthIndex = contextSegments ? -1 : 17;
    const refVideoStrengthIndex = contextSegments ? -1 : 18;
    const refStrengthsIndex = contextSegments ? -1 : 19;

    const rawEnableDetail = named["enable_detail_daemon"] !== undefined
        ? named["enable_detail_daemon"]
        : (enableDetailDaemonIndex >= 0 ? values[enableDetailDaemonIndex] : undefined);
    const parsedEnableDetail = strictBoolean(rawEnableDetail);
    const resolvedEnableDetail = parsedEnableDetail !== null ? parsedEnableDetail : defaults.enable_detail_daemon;

    const rawDetailStrength = named["detail_strength"] !== undefined
        ? named["detail_strength"]
        : (detailStrengthIndex >= 0 ? values[detailStrengthIndex] : undefined);
    const parsedDetailStrength = Number(rawDetailStrength);
    const resolvedDetailStrength = Number.isFinite(parsedDetailStrength)
        ? parsedDetailStrength
        : defaults.detail_strength;

    // refImageStrengthIndex/refVideoStrengthIndex are -1 on the Context
    // Segments node (it has no positional slot tracked for these two by this
    // function). Falling back straight to `defaults` there discarded
    // whatever the widget was actually set to on every repair pass (e.g.
    // every workflow-tab switch), silently resetting it to 1.0. Fall back to
    // the widget's own current live value first -- already positionally
    // correct courtesy of the base onConfigure that ran before this
    // function -- and only use the hard default if that's unavailable too.
    const rawRefImageStrength = named["ref_image_strength"] !== undefined
        ? named["ref_image_strength"]
        : (refImageStrengthIndex >= 0 ? values[refImageStrengthIndex] : getWidget(node, "ref_image_strength")?.value);
    const parsedRefImageStrength = Number(rawRefImageStrength);
    const resolvedRefImageStrength = Number.isFinite(parsedRefImageStrength)
        ? parsedRefImageStrength
        : defaults.ref_image_strength;

    const rawRefVideoStrength = named["ref_video_strength"] !== undefined
        ? named["ref_video_strength"]
        : (refVideoStrengthIndex >= 0 ? values[refVideoStrengthIndex] : getWidget(node, "ref_video_strength")?.value);
    const parsedRefVideoStrength = Number(rawRefVideoStrength);
    const resolvedRefVideoStrength = Number.isFinite(parsedRefVideoStrength)
        ? parsedRefVideoStrength
        : defaults.ref_video_strength;

    const rawRefStrengths = named["ref_strengths"] !== undefined
        ? named["ref_strengths"]
        : (refStrengthsIndex >= 0 ? values[refStrengthsIndex] : undefined);
    const resolvedRefStrengths = typeof rawRefStrengths === "string"
        ? rawRefStrengths
        : defaults.ref_strengths;
    ...
    const normalized = {
        ...
        continuity_mode: Object.prototype.hasOwnProperty.call(OPTION_DEFS.continuity_mode, canonicalOption("continuity_mode", values[continuityModeIndex]))
            ? canonicalOption("continuity_mode", values[continuityModeIndex]) : CONTINUITY_NATIVE,
        transition_seconds: (() => {
            const raw = named["transition_seconds"] !== undefined
                ? named["transition_seconds"]
                : (transitionSecondsIndex >= 0 ? values[transitionSecondsIndex] : 0.0);
            const parsed = Number(raw);
            return Number.isFinite(parsed) ? Math.min(5.0, Math.max(0.0, parsed)) : 0.0;
        })(),
        prompt_optimizer_settings: false,
        ...
        enable_detail_daemon: resolvedEnableDetail,
        detail_strength: resolvedDetailStrength,
        ref_image_strength: resolvedRefImageStrength,
        ref_video_strength: resolvedRefVideoStrength,
        ref_strengths: resolvedRefStrengths,
    };
    for (let i = 1; i <= 9; i += 1) {
        const n = `ref_image${i}_strength`;
        const val = named[n] !== undefined ? Number(named[n]) : (Number(getWidget(node, n)?.value) || 1.0);
        normalized[n] = Number.isFinite(val) ? val : 1.0;
    }
    for (let v = 1; v <= 3; v += 1) {
        const n = `ref_video${v}_strength`;
        const val = named[n] !== undefined ? Number(named[n]) : (Number(getWidget(node, n)?.value) || 1.0);
    ...
    if (contextSegments) {
        ...
        setConfiguredWidgetValue(node, "transition_seconds", normalized.transition_seconds);
        ...
        // Every entry below custom_ratio, and everything from ref_image_strength
        // through seed_video_reserved_seconds, used to be left out of this
        // rebuilt array entirely. Since the base onConfigure (which already ran
        // before this function) assigns a saved workflow's widgets_values back
        // to node.widgets purely by position, and this rebuilt array replaces
        // the one that assignment will use on the *next* onConfigure (e.g. the
        // next workflow-tab switch), any gap here reappears as either a
        // one-slot shift of every later widget (the missing custom_ratio slot)
        // or a widget silently falling back to its default (the missing tail
        // widgets, most visibly seed_video_reserved_seconds resetting to its
        // default). Every fallback in this block must stay equal to the Python
        // INPUT_TYPES default, or a widget silently changes value on tab switch.
        // Keep this the exact same order as MiniMaxH3EasyContextSegments'
        // Python INPUT_TYPES so the positions line up.
        const customRatioValue = typeof getWidget(node, "custom_ratio")?.value === "string"
            ? getWidget(node, "custom_ratio").value
            : (typeof values[5] === "string" ? values[5] : "16:9");
        const refVideoRangeValue = (name, index) => {
            const current = getWidget(node, name)?.value;
            if (typeof current === "string") return current;
            return typeof values[index] === "string" ? values[index] : "";
        };
        // Resolve these by NAME before falling back to positional math.
        // contextTransitionOffset infers the layout from typeof values[13],
        // which assumes continuity_mode sits at [11]. A saved workflow that
        // carries an extra widget slot earlier in the array (an unset optional
        // input, for example) pushes continuity_mode to [12] and
        // transition_seconds to [13]; the type sniff then reads a number where
        // it expected a boolean, returns 0 instead of 1, and every later index
        // lands one slot early -- seed_video_strength picking up a neighbouring
        // 0 instead of its real value. widgets_values_named is keyed by widget
        // name, so it is immune to that whole class of shift.
        const namedWidgetNumber = (name) => {
            const fromNamed = Number(named?.[name]);
            return Number.isFinite(fromNamed) ? fromNamed : undefined;
        };
        const seedVideoStrengthValue = (() => {
            const current = Number(getWidget(node, "seed_video_strength")?.value);
            if (Number.isFinite(current)) return current;
            const fromNamed = namedWidgetNumber("seed_video_strength");
            if (fromNamed !== undefined) return fromNamed;
            const raw = Number(values[31 + contextTransitionOffset]);
            return Number.isFinite(raw) ? raw : 1.0;
        })();
        const seedVideoReservedSecondsValue = (() => {
            const current = Number(getWidget(node, "seed_video_reserved_seconds")?.value);
            if (Number.isFinite(current)) return current;
            const fromNamed = namedWidgetNumber("seed_video_reserved_seconds");
            if (fromNamed !== undefined) return fromNamed;
            const raw = Number(values[32 + contextTransitionOffset]);
        ...
        info.widgets_values = [
            ...
            normalized.enable_detail_daemon,
            normalized.detail_strength,
            normalized.ref_image_strength,
            normalized.ref_video_strength,
            normalized.ref_video1_strength,
            normalized.ref_video2_strength,
            normalized.ref_video3_strength,
            refVideoRangeValue("ref_video1_range", 28 + contextTransitionOffset),
            refVideoRangeValue("ref_video2_range", 29 + contextTransitionOffset),
            refVideoRangeValue("ref_video3_range", 30 + contextTransitionOffset),
            seedVideoStrengthValue,
            seedVideoReservedSecondsValue,
        ];
        // These two were only ever appended to the positional array, never
        // recorded by name. The array written here is shorter than the one a
        // saved workflow can carry, so on reload every trailing value lands one
        // slot early and seed_video_strength's value surfaces as
        // seed_video_reserved_seconds. Recording them by name makes the
        // name-based read authoritative and ends the drift.
        normalized.seed_video_strength = seedVideoStrengthValue;
        normalized.seed_video_reserved_seconds = seedVideoReservedSecondsValue;
        info.widgets_values_named = {
            ...(info.widgets_values_named && typeof info.widgets_values_named === "object" ? info.widgets_values_named : {}),
            ...normalized,
        };
...
// ---------------------------------------------------------------------
// Per-widget "?" help icons: a small clickable circle drawn at the right
// edge of a widget's row, opening a plain-language popup explaining what
// the setting does and what each choice means. Hover tooltips (from the
// Python "tooltip" field) stay as-is; this is for the settings confusing
// enough to need more room than a tooltip gives.
// ---------------------------------------------------------------------
const WIDGET_HELP = {
    mode: () => ({
        title: "Mode",
        body: "What kind of generation this node does.\n\n"
            + "- Image: text-to-video, optionally anchored by a first_frame and/or last_frame image (see Keyframe role).\n"
            + "- Reference: uses connected @Image/@Video/@Audio media as style/character/environment references via @-mentions in the prompt.\n"
            + "- Digital Human: drives a talking-head style generation from a source audio track.",
    }),
    seconds: (node) => (isSegmentMode(node)
        ? {
            title: "Seconds",
            body: "How long the generated video is, in seconds. This is the one duration control for "
                + "this node. If your prompt uses the [Shot N] / 'At MM:SS.mmm' cut-time convention, each "
                + "shot's own length comes from its cut time in the text (Shot 1 starts at 0, later shots "
                + "run until the next one's cut time, and the last shot runs until this total). The prompt itself is the real per-shot duration control; this box only sets where the "
                + "timeline ends. A single undivided prompt, or one with no cut times, just gets the full "
                + "duration as one shot.\n\n"
                + "A fixed duration means a fixed frame budget. If you ask for a lot of dialogue or action "
                + "in a short duration, the model has to compress it to fit, which can look rushed or "
                + "sped-up. Raise this (or trim what the prompt asks for) if that happens.",
        }
        : {
            title: "Seconds",
            body: "How long the generated video is, in seconds. This directly controls how many frames "
                + "get generated. There is no per-shot breakdown here, since this node produces one "
                + "continuous clip rather than a multi-shot sequence.\n\n"
                + "A fixed duration means a fixed frame budget: if you ask for a lot of dialogue or action "
                + "in a short duration, the model has to compress it to fit, which can look rushed or "
                + "sped-up. Raise this (or trim what the prompt asks for) if that happens.",
        }),
    aspect_ratio: () => ({
        title: "Aspect ratio",
        body: "Sets the shape of the output canvas.\n\n"
            + "- Presets (1:1, 16:9, 9:16, etc.): scaled to fit whatever 'Resolution' budget you've picked.\n"
            + "- Custom: type exact pixel Width/Height yourself. This ignores the Resolution dropdown's "
            + "megapixel budget entirely. Whatever you type is what you get, grid-aligned to the "
            + "nearest 32px. Use this when you need an exact, specific size, e.g. matching another clip.\n"
            + "- Free Ratio: type a shape as 'W:H' in the 'Custom ratio' field below (e.g. '240:136'). This one IS scaled by the Resolution budget, just like the presets, so use it for a "
            + "non-standard shape without working out the pixels yourself.",
    }),
    custom_ratio: () => ({
        title: "Custom ratio",
        body: "Only used when Aspect ratio is set to 'Free Ratio'. Type any shape as 'W:H', e.g. '16:9' "
            + "or '240:136'. It's scaled to fit the Resolution dropdown's megapixel budget automatically, "
            + "then grid-aligned to H3's required 32px canvas. You do not need to work out exact pixels yourself.\n\nIgnored completely for every other Aspect ratio choice.",
    }),
    resolution: () => ({
        title: "Resolution",
        body: "Controls the overall pixel budget (roughly how many total megapixels) of the output. Think of it as a quality and detail dial rather than an exact size. The actual width/height come from "
            + "this budget combined with your chosen Aspect ratio.\n\n"
            + "Higher values give more detail, but generate slower and use more VRAM. 'Custom' bypasses this "
            + "budget entirely and uses the literal Width/Height fields below instead.",
    }),
    width: () => ({
        title: "Width / Height",
        body: "Only actually used when Resolution is set to 'Custom', or Aspect ratio is set to 'Custom' "
            + "(not 'Free Ratio'). In those cases these are the literal final pixel dimensions, grid-aligned to the "
            + "nearest 32px.\n\n"
            + "For every other Resolution/Aspect ratio combination, these fields are ignored. The real size comes from the Resolution budget combined with the Aspect ratio shape.",
    }),
    continuity_mode: () => ({
        title: "Continuity mode",
        body: "How the start of this shot connects to whatever came before it, either the previous "
            + "shot or seed_video.\n\n"
            + "- Native Guide (default): hands the model the real footage from the end of the previous "
            + "clip and lets it generate the continuation itself. Nothing is frozen and nothing is "
            + "thrown away: the clip opens by re-creating that footage, then carries on into your "
            + "prompt. Use this when you intend to join two clips together.\n"
            + "- Latent Guide: a softer pull toward continuing smoothly. The model can drift away "
            + "from the join if the shot's content pushes it to.\n"
            + "- RGB Guide: the same idea applied to pixels instead of latents.\n"
            + "- Soft AV Prefix: locks the opening frames so they match exactly, with the audio eased "
            + "across the join. The locked frames are removed before the video is delivered.\n"
            + "- Hard AV Prefix: the same lock, with the audio cut straight across instead of eased.\n\n"
            + "The AV Prefix modes give you a guaranteed exact match, but because the locked frames "
            + "are removed, the video you get starts at the join itself. Native Guide delivers the "
            + "whole thing, which is why it suits stitching.",
    }),
    context_length: () => ({
        title: "Context frames",
        body: "In Native Guide, how much of the previous clip the model is given to continue from. "
            + "This is also the length of the continuation window: the opening stretch of the new "
            + "clip that re-creates that footage before moving on. All of it is delivered. At 25fps, "
            + "22 frames is about 0.9s and 39 is about 1.6s.\n\n"
            + "More frames give the model more motion to carry forward, at the cost of spending more "
            + "of the clip's length re-creating footage you already have. 22 or 39 is usually right.\n\n"
            + "Two things need to line up with it: set overlap_frames on Stitch Continuation to the "
            + "same number, and keep new action in your prompt until after the window. Anything new "
            + "happening inside the window pulls against the footage the model is trying to continue.\n\n"
            + "In Soft/Hard AV Prefix this is how many frames get locked instead, and those frames "
            + "are removed before delivery. It does nothing in Latent Guide or RGB Guide.",
    }),
    ref_image_size: () => ({
        title: "Ref image size",
        body: "How much detail from your @Image/@Video references actually survives into generation. This is a resolution budget for references, not a crop.\n\n"
            + "- 1K / 1.5K / 2K: downscales the reference to roughly that many total pixels if it's "
            + "bigger (never upscales), preserving the full framing and aspect ratio.\n"
            + "- Match: scales the reference to match your output canvas's pixel area.\n"
            + "- Original: essentially full resolution, only grid-aligned.\n\n"
            + "If character faces or fine detail aren't surviving from your references, try a bigger "
            + "budget (2K or Original).",
    }),
    keyframe_role: () => ({
        title: "Keyframe role",
        body: "Only matters when exactly ONE image is connected in Image mode (not two, not Context "
            + "Segments). It decides whether that single image anchors the START of the video (First) or "
            + "the END of it (Last).\n\n"
            + "With two images connected, it also decides which one is treated as the opening frame and "
            + "which is the closing frame.",
    }),
    reference_mention_mode: () => ({
        title: "Reference mention mode",
        body: "Controls what @-mentions look like in the prompt editor and how they're matched back to "
            + "your connected media.\n\n"
            + "- Index (default): mentions look like @Image1, @Video1 and so on, numbered by connection "
            + "order.\n"
            + "- Filename: mentions use the actual source filename instead, e.g. @superman-cropped.png.",
    }),
    audio_mode: () => ({
        title: "Audio mode",
        body: "Default: audio is generated normally alongside the video.\n\n"
            + "Digital Human: drives the output from a connected source audio track (a real voice/track) "
            + "instead of generating new audio, for lip-synced talking-head style output. Falls "
            + "back to Default automatically if no audio is connected.",
    }),
    seed_video_strength: () => ({
        title: "Seed Video Strength",
        body: "How much of the seed video actually reaches the model.\n\n"
            + "1.0 uses the encoded clip exactly as-is. Below 1.0 it is progressively blended "
            + "toward random noise, so the model is given less and less of your footage to work "
            + "from. At 0.0 nothing of the seed video survives at all: the model receives pure "
            + "noise and the continuation has no environment, character or motion to continue.\n\n"
            + "Leave this at 1.0. Lower it only if you deliberately want the new shot to break "
            + "away from how the seed video ended, and expect continuity to degrade quickly as "
            + "you do.\n\n"
            + "Applies to both the video and the audio taken from seed_video, in every continuity "
            + "mode except RGB Guide. Has no effect without a seed_video connected.",
    }),
    seed_video_reserved_seconds: () => ({
        title: "Seed Context Seconds",
        body: "A minimum length for the continuation window, in seconds. Leave it at 0 unless you "
            + "need a window longer than the Context frames dropdown offers.\n\n"
            + "At 0 it does nothing and Context frames alone decides the window. Above 0 it acts as "
            + "a floor: the window becomes whichever is LARGER, this value or Context frames. It is "
            + "not an independent setting. A value larger than Context frames silently replaces "
            + "it, and a smaller one is ignored.\n\n"
            + "Seconds are rounded UP to the nearest length the model accepts, so small changes "
            + "often do nothing and one small step can jump the window a long way. On the guide "
            + "grid (5/22/39/56/73 frames) at 25fps: 0.8s gives 22 frames (0.88s), 1.0s already "
            + "gives 39 frames (1.56s), 2.0s gives 56 (2.24s), and 2.5s or more gives 73 (2.92s), "
            + "which is the maximum. The AV Prefix modes use a coarser grid of 39/90/141 frames.\n\n"
            + "The resolved frame count is printed to the console on every run. Three things should "
            + "agree with it: this window, the overlap_frames setting on Stitch Continuation, and "
            + "the point in your prompt where new action is allowed to start.",
    }),
    advanced: () => ({
        title: "Advanced options",
        body: "Toggles visibility of the less-commonly-needed settings on this node (fps, Keyframe role, "
            + "Ref image size, Reference mention mode, and similar). Turning it off just hides them from "
            + "view -- it doesn't reset or disable whatever they're currently set to.",
    }),
};

function getHelpEntry(node, widget) {
    const factory = WIDGET_HELP[widget?.name];
    if (!factory) return null;
    try {
        return factory(node, widget);
    } catch {
        return null;
    }
}

function drawHelpIcons(node, ctx) {
    if (!Array.isArray(node.widgets)) return;
    const radius = 7;
    // Sits close to the node's true right edge (a small margin, not out in
    // the middle of the widget row) so it doesn't overlap the widget's own
    // value/slider hit area, which used to make it fiddly to land a click on.
    const margin = 3;
    const rightEdge = node.size[0] - margin - radius;
    const hitboxes = [];
    for (const widget of node.widgets) {
        if (!widget || widget.hidden || widget.type === "hidden" || widget.type === "converted-widget") continue;
        if (typeof widget.last_y !== "number") continue;
        const entry = getHelpEntry(node, widget);
        if (!entry) continue;
        const rowHeight = Number(globalThis.LiteGraph?.NODE_WIDGET_HEIGHT) || 20;
        const cy = widget.last_y + rowHeight / 2;
        ctx.save();
        ctx.beginPath();
        ctx.arc(rightEdge, cy, radius, 0, Math.PI * 2);
        ...
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText("?", rightEdge, cy + 0.5);
        ctx.restore();
        // The drawn circle is only 14px across, which is a thin target to
        // land a click on. Pad the actual hit-test box well beyond what's
        // drawn so a slightly-off click still registers.
        const hitPadding = 8;
        const hitRadius = radius + hitPadding;
        hitboxes.push({ x: rightEdge - hitRadius, y: cy - hitRadius, w: hitRadius * 2, h: hitRadius * 2, entry });
    }
    node.__h3HelpHitboxes = hitboxes;
}

function hitTestHelpIcons(node, localPos) {
    const hitboxes = node.__h3HelpHitboxes;
    if (!Array.isArray(hitboxes) || !localPos) return null;
    const [x, y] = localPos;
    for (const box of hitboxes) {
        if (x >= box.x && x <= box.x + box.w && y >= box.y && y <= box.y + box.h) return box.entry;
    }
    return null;
}

function showHelpPopup(title, body) {
    document.getElementById("h3-help-popup-overlay")?.remove();
    const overlay = document.createElement("div");
    overlay.id = "h3-help-popup-overlay";
    ...
    Object.assign(panel.style, {
        background: "#1e1e1e", color: "#e8e8e8", borderRadius: "8px",
        maxWidth: "480px", width: "90%", maxHeight: "70vh",
    ...
    // The "?" button opens this popup on mousedown (see onMouseDown below).
    // The matching mouseup of that same click still lands on whatever
    // element is now under the cursor -- which, the instant the overlay is
    // inserted, is the overlay itself, covering the full viewport. That
    // mouseup+mousedown pair fires a native "click" event on the overlay
    // right away, and a click listener attached synchronously here used to
    // catch that same click and immediately remove(), so the popup opened
    // and closed within one click and never appeared to show anything.
    // Attaching the outside-click listener on the next tick lets the click
    // that opened the popup finish first.
    setTimeout(() => {
...
function installNode(nodeType, nodeData) {
    ...
    nodeType.prototype.onNodeCreated = function onNodeCreatedH3Easy() {
        ...
        const aspectRatioWidget = getWidget(this, "aspect_ratio");
        if (aspectRatioWidget && !aspectRatioWidget.__h3ConditionalCallbackBound) {
            aspectRatioWidget.__h3ConditionalCallbackBound = true;
        ...
        const enableContextSegmentsWidget = getWidget(this, "enable_context_segments");
        if (enableContextSegmentsWidget && !enableContextSegmentsWidget.__h3ConditionalCallbackBound) {
            enableContextSegmentsWidget.__h3ConditionalCallbackBound = true;
            const originalCallback = enableContextSegmentsWidget.callback;
            enableContextSegmentsWidget.callback = (value) => {
                originalCallback?.call(enableContextSegmentsWidget, value);
                syncModeWidgets(this);
                repairNodeLayout(this);
                this.setDirtyCanvas?.(true, true);
            };
        ...
        const enableDetailDaemonWidget = getWidget(this, "enable_detail_daemon");
        if (enableDetailDaemonWidget && !enableDetailDaemonWidget.__h3ConditionalCallbackBound) {
            enableDetailDaemonWidget.__h3ConditionalCallbackBound = true;
            const originalCallback = enableDetailDaemonWidget.callback;
            enableDetailDaemonWidget.callback = (value) => {
                originalCallback?.call(enableDetailDaemonWidget, value);
                if (enableDetailDaemonWidget._state) enableDetailDaemonWidget._state.value = value;
                syncModeWidgets(this);
                repairNodeLayout(this);
                this.setDirtyCanvas?.(true, true);
            };
    ...
    nodeType.prototype.onConnectionsChange = function onConnectionsChangeH3Easy(type, index, connected, linkInfo) {
        ...
        if (/^seed_(?:video|ref_video)$/.test(String(input?.name || ""))) requestMentionPreviewRefresh();
        if (/^media(?:_\d+)?$/i.test(String(input?.name || ""))) {
            // Connecting or disconnecting a reference renumbers and re-resolves
            // every @ImageN in the prompt, but nothing re-rendered the editor,
            // so already-typed tokens stayed grey and unresolved until the
            // prompt was pasted again. Re-render on the next frame, once the
            // link tables have settled. renderEditorFromNode is called without
            // force, so it returns early while the editor has focus and can
            // never interrupt an edit in progress.
            requestMentionPreviewRefresh();
            globalThis.requestAnimationFrame?.(() => {
                try {
                    if (this.__h3Editor) renderEditorFromNode(this);
                    this.setDirtyCanvas?.(true, true);
                } catch (err) {
                    console.warn("[MiniMax H3 Easy] mention refresh after media change failed", err);
                }
            });
        }
        syncModeWidgets(this);
    ...
    nodeType.prototype.onDrawForeground = function onDrawForegroundH3Easy(ctx) {
        ...
        if (!this.flags?.collapsed) drawHelpIcons(this, ctx);
        return result;
    };

    const originalMouseDown = nodeType.prototype.onMouseDown;
    nodeType.prototype.onMouseDown = function onMouseDownH3Easy(event, localPos, canvas) {
        const entry = hitTestHelpIcons(this, localPos);
        if (entry) {
            showHelpPopup(entry.title, entry.body);
            return true;
        }
